...
from tkinter import messagebox #use to display message boxes in the python appliances 
import speech_recognition as sr  #library to perform speech recognition 
import pyttsx3 #text to speech module
from win32com.client import constants, Dispatch # it is library that enables programmer to use Windows API to create Gui based programmes 
#function to translate the speech to text in python 
def SpeakText(command):

	engine = pyttsx3.init()
	engine.say(command) #intilize the engine command to start as argument 
	engine.runAndWait() # wait for user to terminate the conversation 

Working_Dir = os.getcwd()  #use to getr  the current working directory

r = sr.Recognizer() #uses the machine mic to listen and analyze what has been said 
mic = sr.Microphone(device_index=14)  #which microphone is been used while intitializing the conversation 
...
class Application(tk.Frame):# it is used to organize and group  widgets 
	...
	def Main_Frame(self): # Driver code of the Dialog Box 
		...
		self.Label_1['text'] = 'Convert  Text to Audio And Audio to Text Speech using Python' #Main dialog box 
		self.Label_1.grid(row=0, column=0, pady=30)

		self.Label_2 = tk.Label(self.Frame_1)
		self.Label_2['text'] = 'Requires an Active Internet Connection' #statement displayed on the main dialog box 
		self.Label_2.grid(row=1, column=0, pady=10, padx=100)

		self.SpeehToText = tk.Button(self, bg='#FFFF00', fg='black',font=("Times new roman", 14, 'bold')) #customization of the buttons 
		self.SpeehToText['text'] = 'Speech to Text'#text written on the buttons 
		self.SpeehToText['command'] = self.SpeechToText
		self.SpeehToText.grid(row=1, column=0, pady=80, padx=60)

		self.TextTo_Speech = tk.Button(self, bg='#FFFF00', fg='black',font=("Times new roman", 14, 'bold'))
	...
	def SpeechToText(self):
		...
		self.Listen = tk.Button(self, bg='#FFFF00', fg='black',font=("Times new roman", 18, 'bold'))
		self.Listen['text'] = 'Listen'
		self.Listen['command'] = self.Audio_Recognizer
		self.Listen.grid(row=0, column=0, pady=40)

		self.Back = tk.Button(self, bg='brown', fg='black',font=("Times new roman", 18, 'bold'))
		self.Back['text'] = 'back'
	...
	# 		msg = r.recognize_google(Audio) #r.recognize_google(Audio, language = 'pt')
	# 		self.text.insert('1.0', msg)
	# 	except:
	# 		#msg = r.recognize_google(Audio, language = 'pt')
	# 		#self.text.insert('1.0', msg)
...
root.wm_title('Text to Speech converter Ayush and Aryan')
